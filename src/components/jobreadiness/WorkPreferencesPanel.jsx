...
export default function WorkPreferencesPanel({ resident, profile, staff, residentId, globalId, onRefresh }) {
  ...
  const display = (val, fallback = '—') => (Array.isArray(val) ? (val.length ? val.join(', ') : fallback) : (val || fallback));
      ...
      {editing ? (
        <div className="space-y-4">
          <div className="grid sm:grid-cols-2 gap-4">
            <div>
              <label className="text-xs font-medium text-muted-foreground block mb-1">Preferred Job Types (comma separated)</label>
          ...
          <div>
            ...
            <Textarea value={form.accommodations_needed} onChange={e => setForm(f => ({ ...f, accommodations_needed: e.target.value }))} rows={2} placeholder="Any workplace accommodations..." />
          </div>
          <div>
            <label className="text-xs font-medium text-muted-foreground block mb-1">Barrier-Aware Work Notes (staff only)</label>
